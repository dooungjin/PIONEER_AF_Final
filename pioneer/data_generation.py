...
AF_sasa = pd.read_csv('/fs/cbsuhyfs1/storage/dl953/sasa_perpdb_alltax.txt', sep = '\t')
...
def generate_seq_seq_data(int_list, seq_feat_dir):
    ...
    """
    #colmeans = pd.read_pickle(pkg_resources.resource_filename(__name__, 'data/mean_values/sequence_feature_colmean.pkl'))[1:]
    #DL: fillna not working
...
def generate_models_to_use(int_list, seq_dict, exclude_hom_struct=False, min_coverage=0.1):
    """
    ...
    #DL: Some lines below are commented out to facilitate the process. The result of the below part will be loaded at the end of the comments.
    #DL: human_modbase_index = '/local/storage/sl2678/interface_pred/pipeline/pipeline_data/modbase/parsed_files/m3D_modbase_models.txt'
    #DL: It has the information of modbase models
    ...
    # Load UniProt sequence length dictionary.
    #DL: UNIPROT_LENGTH_PICKLE_PATH is '/local/storage/sl2678/interface_pred/pipeline/pipeline_data/uniprot_length_dict.pkl'
    #DL: It is a dictionary mapping from UniProt to length. Ex) A0A2G8S617': 106, 'A0A2G8S710': 87
    with open(UNIPROT_LENGTH_PICKLE_PATH, 'rb') as f:
        uniprot_length_dict = pickle.load(f)
    
    # Concatenate the ModBase and PDB dataframes.
    full_model_index = pd.concat([human_modbase_index, nonhuman_modbase_index, pdb_model_index], ignore_index=True, sort=True)
    full_model_index = full_model_index[full_model_index['uniprot'].isin(uniprot_length_dict)]
    ...
    full_model_index['coverage'] = full_model_index.apply(lambda x: float(len(unzip_res_range(x['uniprot_res']))) / uniprot_length_dict[x['uniprot']], axis=1) 
    ...
    #DL: This is one of arguments of this function. For prediction, it is False
    if exclude_hom_struct:
        with open(UNIPROT_SEQ_PICKLE_PATH, 'rb') as  f:
            uniprot_seq_dict = pickle.load(f)
        excluded_pdb_dict = calculate_excluded_pdb_dict(int_list, seq_dict, uniprot_seq_dict)
    else:
...
def get_struct_file(struct_info, out_file_prefix):
    """
    ...
    if stype == 'PDB':
        ...
        if not os.path.exists(os.path.join(MODBASE_CACHE, 'models', 'hash', struct_info[3][:-3])):
            return
...
def obtain_all_structures(int_list, models_to_use, out_dir):
    """
    ...
    for idx, (p1, p2) in enumerate(int_list):
        ...
        #if ((p1, p2) not in models_to_use) or (len(models_to_use[(p1, p2)]) == 0): # if none of proteins in interaction has structure information, such interaction does not exist in models_to_use
        ...
        if p1 == p2: # Homodimer
            ...
            for i, p1_model in enumerate(p1_models):
                get_struct_file(p1_model, os.path.join(out_dir, '_'.join([p1, p2, str(i), '0'])))
        else: # Heterodimer
            if len(model_dict) == 1: # One of p1 and p2 has structural information.
                ...
                if p == p1: # Only p1 has structure information
                    ...
                    for i, p2_model in enumerate(p2_models):
                        get_struct_file(p2_model, os.path.join(out_dir, '_'.join([p1, p2, str(i), '1'])))
...
def generate_seq_str_data(int_list, seq_feat_dir, full_feat_dir, struct_dir, p2=False, min_res_num=11, AF_threshold = 90):
    """
    ...
    for idx, (p1, p2) in enumerate(int_list):
        try:
            ...
            p1_feature = pd.DataFrame(p1_feature, columns = seq_colnames)
            #p1_feature = p1_feature.fillna(seq_colmeans)
            p1_sample = list(range(p1_feature.shape[0]))
            ...
            if os.path.exists(os.path.join(DISTANCE_MATRIX_CACHE, dm_fname)): # If distance matrix has been pre-calculated
                ...
                # TODO: save newly calculated distance matrix to cache? Done
                p2_edge_index, new_dm = get_edge_coo_data(p2_pdb_array, 10)
                np.save(DISTANCE_MATRIX_CACHE + dm_fname, new_dm)
                print("New DM", DISTANCE_MATRIX_CACHE + dm_fname)
            ...
            #p2_feature = p2_feature.fillna(full_colmeans)
...
def generate_str_seq_data(int_list, seq_feat_dir, full_feat_dir, struct_dir, p2=False, min_res_num=11, AF_threshold = 90):
    """
    ...
    for p1, p2 in int_list:
        try:
            ...
            for p1_struct_f in p1_struct_files:
                ...
                #p1_feature = p1_feature.fillna(full_colmeans)
                ...
                p2_feature = pd.DataFrame(p2_feature, columns = seq_colnames)
                #p2_feature = p2_feature.fillna(seq_colmeans)
                p2_rnn_feature = p2_feature.values
...
def generate_str_str_data(int_list, full_feat_dir, struct_dir, min_res_num=11, AF_threshold = 90):
    """
