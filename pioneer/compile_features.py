...
def compile_eclair_features(int_list, seq_dict, out_dir, exclude_hom_struct=False):
    """
    Calculate ECLAIR features and compile one dataframe in pickle format for each protein of an interaction.
    ...
    Args:
        ...
        out_dir (str): Output directory for generated pickle files.
    ...
    js_feat_dict = calculate_conservation(seq_dict)
    sca_feat_dict, dca_feat_dict = calculate_coevolution(int_list, seq_dict)
    ...
    if exclude_hom_struct:
        _, uniprot_seq_dict = parse_fasta(UNIPROT_ALL_DB)
        uniprot_seq_dict = {k.split('|')[1]:v for k, v in uniprot_seq_dict.items()}
        excluded_pdb_dict = calculate_excluded_pdb_dict(struct_int_list, struct_seq_dict, uniprot_seq_dict)
    else:
        excluded_pdb_dict = defaultdict(set)
    _, sasa_mean_dict = calculate_sasa(struct_int_list, struct_seq_dict, excluded_pdb_dict)
    zdock_feat_dict = calculate_zdock(struct_int_list, struct_seq_dict, excluded_pdb_dict)

    # Step 2: Compile ECLAIR feature dataframes.
    for i in int_list:
        p1, p2 = i
        p1_features = pd.DataFrame()
        p2_features = pd.DataFrame()
        
        for feat in ['AREA', 'HPHO', 'POLA', 'TRAN']: # ExPaSy features
            p1_features['expasy_%s_raw' % feat] = expasy_feat_dict[feat][p1]
            p2_features['expasy_%s_raw' % feat] = expasy_feat_dict[feat][p2]
        
        if p1 in js_feat_dict: # JS features. Need to do normalization and ranking transformations
            p1_js_array = js_feat_dict[p1]
            p1_features['JS_raw'] = p1_js_array
            p1_features['JS_norm'] = normalize(p1_js_array)
            p1_features['JS_rank'] = rank(p1_js_array, 'hi2lo')
        else:
            p1_features['JS_raw'], p1_features['JS_norm'], p1_features['JS_rank'] = [np.nan] * p1_features.shape[0], [np.nan] * p1_features.shape[0], [np.nan] * p1_features.shape[0]
        if p2 in js_feat_dict:
            p2_js_array = js_feat_dict[p2]
            p2_features['JS_raw'] = p2_js_array
            p2_features['JS_norm'] = normalize(p2_js_array)
            p2_features['JS_rank'] = rank(p2_js_array, 'hi2lo')
        else:
            p2_features['JS_raw'], p2_features['JS_norm'], p2_features['JS_rank'] = [np.nan] * p2_features.shape[0], [np.nan] * p2_features.shape[0], [np.nan] * p2_features.shape[0]
        
        if i in sca_feat_dict['max']: # SCA and DCA features. Need to do normalization
            p1_sca_array, p2_sca_array = sca_feat_dict['max'][i]
            p1_features['SCA_max_norm'] = normalize(p1_sca_array)
            p2_features['SCA_max_norm'] = normalize(p2_sca_array)
        else:
            p1_features['SCA_max_norm'], p2_features['SCA_max_norm'] = [np.nan] * p1_features.shape[0], [np.nan] * p2_features.shape[0]
        if i in dca_feat_dict['DMI_max']:
            p1_dmi_array, p2_dmi_array = dca_feat_dict['DMI_max'][i]
            p1_features['DMI_max_norm'] = normalize(p1_dmi_array)
            p2_features['DMI_max_norm'] = normalize(p2_dmi_array)
            p1_ddi_array, p2_ddi_array = dca_feat_dict['DDI_max'][i]
            p1_features['DDI_max_norm'] = normalize(p1_ddi_array)
            p2_features['DDI_max_norm'] = normalize(p2_ddi_array)
        else:
            p1_features['DMI_max_norm'], p2_features['DMI_max_norm'] = [np.nan] * p1_features.shape[0], [np.nan] * p2_features.shape[0]
            p1_features['DDI_max_norm'], p2_features['DDI_max_norm'] = [np.nan] * p1_features.shape[0], [np.nan] * p2_features.shape[0]
        
        if i in sasa_mean_dict: # SASA features. Need to do normalization
        ...
        if i in zdock_feat_dict['max']: # ZDOCK feature. Need to do normalization
            p1_zdock_array, p2_zdock_array = zdock_feat_dict['max'][i]
            p1_features['zDOCK_dist3d_PRIORITY_0c_max_norm'], p2_features['zDOCK_dist3d_PRIORITY_0c_max_norm'] = normalize(p1_zdock_array), normalize(p2_zdock_array)
        else:
            p1_features['zDOCK_dist3d_PRIORITY_0c_max_norm'], p2_features['zDOCK_dist3d_PRIORITY_0c_max_norm'] = [np.nan] * p1_features.shape[0], [np.nan] * p2_features.shape[0]
        
        feature_order = ['expasy_AREA_raw', 'expasy_HPHO_raw', 'expasy_POLA_raw', 'expasy_TRAN_raw', 'JS_raw', 'JS_norm', 'JS_rank', 'SCA_max_norm', 'DMI_max_norm', 'DDI_max_norm', 'SASA_combined_avg_raw', 'SASA_combined_avg_norm', 'zDOCK_dist3d_PRIORITY_0c_max_norm']
        p1_features[feature_order].to_pickle(os.path.join(out_dir, '%s_%s_0.pkl' % (p1, p2)))
        if p1 != p2: 
            p2_features[feature_order].to_pickle(os.path.join(out_dir, '%s_%s_1.pkl' % (p1, p2)))

def compile_pioneer_seq_feats(int_list, seq_dict, out_dir):
    """
    ...
    raptorx_dict = calculate_raptorX(seq_dict)
    print("Seq feat, RaptorX finished")
    pp_dict, pp_norm_dict = calculate_pair_potential(int_list, seq_dict)
...
def compile_pioneer_full_feats(int_list, seq_dict, out_dir, exclude_hom_struct=False):
    """
    ...
    # Step 1: Calculate all features.
    expasy_feat_dict = calculate_expasy(seq_dict)
    print("Str feat, Expasy finished")
    js_feat_dict = calculate_conservation(seq_dict)
    print("Str feat, JS conservation finished")
    sca_feat_dict, dca_feat_dict = calculate_coevolution(int_list, seq_dict)
    print("Str feat, SCA, DCA finished")
    ...
    if exclude_hom_struct:
        ...
        excluded_pdb_dict = defaultdict(set)
    sasa_max_dict, sasa_mean_dict = calculate_sasa(struct_int_list, struct_seq_dict, excluded_pdb_dict)
    print("Str feat, SASA finished")
    zdock_feat_dict = calculate_zdock(struct_int_list, struct_seq_dict, excluded_pdb_dict)
    print("Str feat, ZDOCK finished")
    raptorx_dict = calculate_raptorX(seq_dict)
    print("Str feat, RaptorX finished")
    pp_dict, pp_norm_dict = calculate_pair_potential(int_list, seq_dict)
    print("Str feat, PairPotential finished")
